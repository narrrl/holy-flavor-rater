...
const AdminUserDetail: React.FC = () => {
  ...
  useTitle(
    user ? `${t("admin.manageUser")}: ${user.username}` : "User Management",
  );

  const handleToggleActive = async () => {
    if (!user) return;
    try {
      await api.patch(`admin-custom/${id}/user_detail/`, {
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        "PERMANENTLY DELETE THIS USER AND ALL DATA? This cannot be undone.",
      )
    )
      return;
    try {
            ...
            <Grid size={{ xs: 12, sm: 8, md: 9 }}>
              <Box sx={{ textAlign: { xs: "center", sm: "left" } }}>
                ...
                <Typography
                  ...
                  sx={{ mt: 0.5 }}
                >
                  Joined: {formatDate(user.date_joined)} • Last Login:{" "}
                ...
                >
                  ...
                  />
                  {user.is_superuser && (
                    <Chip label="Superuser" color="secondary" size="small" />
                  )}
                </Stack>
              </Box>
            </Grid>
      ...
      {activeTab === 2 && (
        ...
        >
          ...
          <Typography variant="body1" sx={{ mb: 4, maxWidth: 600 }}>
            Deleting this user will permanently remove their profile, all their
            ratings, replies, and IP history.
