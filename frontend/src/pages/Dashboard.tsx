...
const Dashboard: React.FC = () => {
  ...
  const [exploreSort, setExploreSort] = useState<"community" | "circle">(
    "community",
  );
  ...
  const shareUrl = data
    ? `${window.location.origin}/profile/${data.user.username}`
    : "";
  ...
  const handleUpdateRating = async (ratingId: number) => {
    try {
      await api.patch(`ratings/${ratingId}/`, {
        score: editScore,
        comment: editComment,
      });
  ...
  const filteredMissing = useMemo(() => {
    ...
    let items = data.missing_flavors.filter((f) => {
      ...
      const matchCat =
        exploreCategory === "All" || f.category_name === exploreCategory;
      const matchSearch = f.name
        .toLowerCase()
        .includes(exploreSearch.toLowerCase());
    ...
    if (exploreSort === "community") {
      ...
      items.sort(
        (a, b) =>
          (b.followed_average_rating || 0) - (a.followed_average_rating || 0),
      );
    }
    return items;
  ...
  const filteredAndSortedRated = useMemo(() => {
    ...
    let items = [...data.my_ratings].filter((r) => {
      const matchCat =
        ratedCategory === "All" || r.category_name === ratedCategory;
      const matchSearch = r.flavor_name
        .toLowerCase()
        .includes(ratedSearch.toLowerCase());
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          <Typography
            variant="h5"
            sx={{ fontWeight: "900", display: { xs: "none", md: "block" } }}
          >
            {t("dashboard.title")}
          </Typography>
        </Box>
                    ...
                    {editingRatingId === rating.id ? (
                      ...
                      >
                        <Typography
                          ...
                          sx={{ fontWeight: "bold" }}
                        >
                          {t("dashboard.editingReview", {
                            flavor: rating.flavor_name,
                          })}
                        </Typography>
                        <Stack direction="row" spacing={2} alignItems="center">
                          ...
                          <MuiRating
                            max={10}
                            value={editScore}
                            onChange={(_: any, val: number | null) =>
                              setEditScore(val || 0)
                            }
                          />
                        </Stack>
