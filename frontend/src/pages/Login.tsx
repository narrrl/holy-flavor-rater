...
const Login: React.FC<LoginProps> = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMsg =
        err.response?.data?.non_field_errors?.[0] || t("auth.loginFailed");
      if (
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      await api.post("users/verify_signup/", {
        username,
  ...
  const handleRequestReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "If an account exists, a reset code has been sent.",
      });
    } catch (err: any) {
      setMessage({
        type: "error",
        ...
        <form onSubmit={handleVerify}>
          ...
          <Typography
            ...
            dangerouslySetInnerHTML={{
              __html: t("auth.verifyDesc", { username }),
            }}
          />
          <TextField
            fullWidth
