...
const Support: React.FC = () => {
  ...
  const [unreadTicketIds, setUnreadTicketIds] = useState<Set<number>>(
    new Set(),
  );
      ...
      const unreadIds = new Set<number>(
        notifs
          .filter(
            (n: any) =>
              !n.is_read &&
              n.notification_type.startsWith("ticket") &&
              n.ticket,
          )
          .map((n: any) => n.ticket),
      );
          ...
          {tickets.map((ticket: Ticket) => (
            <Card
              ...
              sx={{
                borderRadius: 3,
                border: "1px solid",
                borderColor: unreadTicketIds.has(ticket.id)
                  ? "primary.main"
                  : "divider",
                        ...
                        {["open", "in_progress", "resolved", "closed"].map(
                          (s) => (
                            <Button
                              ...
                              size="small"
                              variant={
                                ticket.status === s ? "contained" : "outlined"
                              }
