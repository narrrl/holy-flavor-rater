...
const FlavorList: React.FC = () => {
  ...
  const [ratingInput, setRatingInput] = useState<{
    [key: number]: { score: number; comment: string };
  }>({});
  ...
  const handleRate = async (flavorId: number) => {
    ...
    try {
      ...
      alert(
        "Failed to submit rating. Make sure you are logged in and haven't rated this yet.",
      );
    }
  };
                        ...
                        <Accordion variant="outlined" sx={{ mt: 1, mb: 1 }}>
                          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                            <Typography variant="caption">
                              Comments (
                              {flavor.ratings.filter((r) => r.comment).length})
                            </Typography>
                          </AccordionSummary>
                          <AccordionDetails
