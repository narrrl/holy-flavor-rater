...
const MainPage: React.FC<MainPageProps> = ({ adminMode }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        if (query) {
          ...
          setNewestFlavors(
            Array.isArray(results[2].data)
              ? results[2].data
              : results[2].data.results || [],
          );
          setCategories(
            Array.isArray(results[3].data)
              ? results[3].data
              : results[3].data.results || [],
          );
  ...
  const handlePrev = () => {
    setActiveIndex(
      (prev) => (prev - 1 + topFlavors.length) % topFlavors.length,
    );
    startTimer();
  };
  const handleNext = () => {
        ...
        >
          ...
          {topFlavors.length > 0 && currentTop && (
            <Box sx={{ flex: { xs: "1 1 100%", xl: "1 1 65%" }, minWidth: 0 }}>
              ...
              <Box
                sx={{ position: "relative", minHeight: { xs: 400, md: 500 } }}
              >
                {categoryLoading && (
                  <Box
                    sx={{
                ...
                >
                  ...
                  >
                    ...
                    >
                      <RatingBadge
                        score={currentTop.average_rating || 0}
                        size="large"
                      />
                      <Typography
                        variant="body2"
                    ...
                    {featuredReview && (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {!featuredReview.user_avatar &&
                              featuredReview.user.charAt(0)}
                          </Avatar>
                          <Typography
                            variant="caption"
                            color="text.secondary"
