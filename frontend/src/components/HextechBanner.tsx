...
const HextechBanner: React.FC<HextechBannerProps> = ({
  username,
  palette,
  settings,
}) => {
  ...
  useEffect(() => {
    ...
    const createStream = (width: number, height: number) => {
      const color =
        brightColors[Math.floor(pseudoRandom(time) * brightColors.length)];
      ...
      for (let y = -gridSize; y < height + gridSize; y += h / 2) {
        for (let x = -gridSize; x < width + gridSize; x += w * 0.75) {
          ...
          const dist = Math.sqrt(
            Math.pow(finalX - mouse.x, 2) + Math.pow(finalY - mouse.y, 2),
          );
          const mouseImpact =
            Math.max(0, (200 - dist) / 200) * (mouse.down ? 2 : 1);

          const dx =
            Math.sin(time + finalX * 0.01) *
            gridSize *
            distortionFactor *
            (1 + mouseImpact);
          const dy =
            Math.cos(time + finalY * 0.01) *
            gridSize *
            distortionFactor *
            (1 + mouseImpact);
          ...
          // Pulse glow
          const pulseVal =
            0.1 + Math.sin(time * 2 + (finalX + finalY) * 0.005) * 0.1;
