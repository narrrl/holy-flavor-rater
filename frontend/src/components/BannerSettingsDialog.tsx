...
const BannerSettingsDialog: React.FC<BannerSettingsDialogProps> = ({
  open,
  onClose,
  banner,
  onSave,
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers>
        ...
        {banner?.schema && banner.schema.length > 0 ? (
          <Box sx={{ mt: 1 }}>{banner.schema.map((field) => renderField(field))}</Box>
        ) : (
          <Typography color="text.secondary" sx={{ fontStyle: 'italic', py: 2 }}>
            No dynamic schema defined for this banner. Please use the JSON editor or check the
      ...
      <DialogActions sx={{ px: 3, py: 2 }}>
        ...
        <Button onClick={handleSave} variant="contained" disabled={saving} sx={{ px: 4 }}>
