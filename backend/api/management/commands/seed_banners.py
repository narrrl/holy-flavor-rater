                ...
                try:
                    ...
                    if not created:
                        ...
                        if updated:
                            ...
                            self.stdout.write(
                                self.style.SUCCESS(
                                    f"Synced new settings for banner '{slug}' from {filename}"
                                )
                            )
                    else:
                        self.stdout.write(
                            self.style.SUCCESS(f"Created new banner '{slug}' from {filename}")
                        )

                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f"Failed to sync banner from {filename}: {e}")
                    )
