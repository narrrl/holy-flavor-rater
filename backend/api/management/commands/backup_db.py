...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--retention", type=int, default=7, help="Number of days to keep old backups"
        )
        parser.add_argument(
            "--full", action="store_true", help="Create a full archive including media"
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["full"]:
            ...
            try:
                ...
                self.stdout.write(self.style.SUCCESS("Full archive created successfully."))
            except Exception as e:
            ...
            if os.path.isfile(file_path):
                ...
                if file_time < cutoff:
                    try:
                        ...
                        self.stdout.write(
                            self.style.WARNING(f"  -> Failed to delete {filename}: {e}")
                        )
